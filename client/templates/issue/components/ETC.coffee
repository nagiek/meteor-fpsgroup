...
Template.ETC.helpers
  ...
  displayAmount: ->
    numeral(@amount).format(i18n("common.numbers.formats.currency")) if @amount? and not isNaN(@amount)
...
Template.issue.validators.etcs = (etcs) ->

  return unless etcs

  withProperType = (etc) ->
    etc.fromNum = Number(etc.fromNum)
    etc.toNum = Number(etc.toNum)
    etc.amount = Number(etc.amount)
    etc

  empty = (etc) -> not etc.fromNum or not etc.toNum or not etc.amount
